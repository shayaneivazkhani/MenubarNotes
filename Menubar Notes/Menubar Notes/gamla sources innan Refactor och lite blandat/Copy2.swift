 ...
 */
     ...
     @State private var p0StarterPageWith4Buttons = true
         @State private var p1showPageIMPandURGPopUp = false
         @State private var p2showPageIMPPopUp = false
         @State private var p3showPageURGPopUp = false
         @State private var p4showPageNOTHINGPopUp  = false
     
 // MARK: – For Keeping Count of How Many Items There is on Different Pages ・・・・・・・・・・・・・・・
     
     @State var isThereAnyIMPandURGItem = false
     @State var isThereMoreThanOneIMPandURGItem = false
     
     @State var isThereAnyIMPItem = false
     @State var isThereMoreThanOneIMPItem = false
     
     @State var isThereAnyURGItem = false
     @State var isThereMoreThanOneURGItem =  false
     
     @State var isThereAnyNOTHINGItem = false
     @State var isThereMoreThanOneNOTHINGItem = false
 ...
 */
     ...
     @State var ShowTextFieldTaskNameFilterResultTextHint: String = ""
     @State var ShowTextFieldTaskDescriptionFilterResultTextHint: String = ""
     @State var ShowCONTAINSTaskName: String = ""
     @State var ShowCONTAINSTaskDescription: String = ""
     @State var helpWithShowCONTAINS: String = ""
     
     @State var ShowRequirementOptions = true
     ...
     @State var ShowShowBarForTaskNameIfTrueElseTaskDescription = true
     @State var uselessButUsedinShowItemsAfterSavingTask = false
     @State var thereIsItemsInCallFromShowButton = false // ❗️används i if $ShowMode.wrappedValue && !($SortMode.wrappedValue) { / Button("Show")
     
     
     
     // MARK: – ⭐️ För Sort
     @State var SortMode = false // if true -> ist för trash och + button så öppnas Sort alternativen
     @State var SortRequirementOptions = true // ska alltid bli assignad till true så att antingen sort By Date knappen / Sort by Priority ska öppnas/visas
     
     @State var TasksIsSortedbyDateIfTrueElsePriority = true // ❗️
     @State var TasksIsSortedIncreasingIfTrueElseDecreasing = true // ❗️
     @State var SortByIncreasing = false // ❗️if true -> då ska du call the apropiate sort från RealmManager when you edit (add/delete) tasks, och även när du öppnar upp en task item och SAVE den så listan ska vara sorterad på det sättet som det var innan man clicked på¨en item och editor sidan öppnades, ❗️if == false då är listan sorterad dec
     
     
     
     // MARK: – ⭐️ För Sort fast dock efter att vi har använt SHOW på listan
     @State var ShowFilterAndSortMode = false // if true -> då är vi i if !($EditMode.wrappedValue) { ––› if $Showing.wrappedValue && !($Sorted.wrappedValue) {
     @State var showBackButtonForSortinShowingAndSortedIfTrue = false // används för att dölja Unshow knappen i översta HStacken på listan av task itemsen för att annars det finns ej så mycket plats för Sort Options att visas
     
     @State var ShowSortRequirementAfterUnshow = true // if true -> då visas det upp Date/Priority ist för inc/dec i if $Showing.wrappedValue && !($Sorted.wrappedValue) { – i if !($ShowingAndSortedIfTrueElseShowing.wrappedValue) { – i if $isThereMoreThanOneIMPandURGItem.wrappedValue { – i if $ShowFilterAndSortMode.wrappedValue {
     
     
     
     // MARK: – ⭐️ För Move fast dock efter att vi har använt Show&Filter på listan
     @State var showMoveAfterShowFilterAndSortMode = true
     
     
     
 // MARK: – ❗️⭐️❗️ For setting the standard-view-of-items/SHOW/SORT of the list of Items ・・・・・・・・・・・・・・・・・・・・・・・・・・・・・・
     
     @State var Showing = false // ❗️if true -> listan är filtrerad/Showing och visar vissa Items containing a String
         @State var ShowingAndSortedIfTrueElseShowing = false // ❗️if true -> listan är både filtrerad/Showing och sorterad, if false -> listan är endast Filtrerad/Showing
             @State var ShowSortByDateAfterUnshowButtonIfTrueElsePriority = true // ❗️if true -> då kommer vi att använda realmManager.FilterAndSort() snart för att sortera det vi Filtrerade/Show baserad på Task Itemens Datum
             @State var ShowSortByIncreasingAfterUnshowButtonIfTrueElseDecreasing = true // ❗️if true -> då har vi använt realmManager.FilterAndSort() och sorterat det vi Filtrerade/Show sedan innan enligt en Increasing
     @State var Sorted = false  // ❗️if true -> listan är sorterad endast filtrerad/Showing
 ...
 */
 ...
 // MARK: – ⚪️För att Wrap NSTextView in SwiftUI – Hur man kan använda sig av en TextView i macOS⚪️
 ...
 */
 ...
 // MARK: – Hide scroller of NSScrollView (i.e. hide tha fu scrollen för TextEditorn)
